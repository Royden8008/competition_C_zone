...
飞手用 RC(POSCTL/定点)**手动**穿一遍树林(不开自主避障),全程录包。零风险拿到真实树林点云。
...
> 话题名以实际 `rostopic list` 为准(雷达可能是 `/livox/lidar` 或其它命名)。
> **录之前先 `rostopic hz <话题>` 确认有数据再开录**,别录空包。

**为什么 raw + registered 都录:**
- `/cloud_registered`:离线复盘规划/聚类(FAST-LIO 配置已固化进点云)。
- `/livox/lidar` + `/livox/imu`:若怀疑定位本身,可离线**换 FAST-LIO 参数重跑**,这必须有原始数据。
...
rosbag play --clock c_zone_xxx.bag           # --clock 让全系统用包里时间戳
rviz -d $(rospack find safe_landing)/rviz/safe_landing.rviz
```

### 1.3 ⚠️ 能查什么 / 不能查什么(关键认知)
